...
It defaults to **0.0 (off)**, and the probability is tested before the RNG is drawn, so a run with it off draws exactly what it drew before the operator existed. That is why the determinism goldens are unchanged and why archived configs, which omit the key, still mean what they meant — this is a new key that does *not* join the "Config vintage" note. The default is off rather than 0.05 because deletion is monotonically good for realizability: the status objective pays for it while the similarity objectives do not, so under NSGA-II a gutted candidate is non-dominated and sits on the front. Whether that costs repair quality is a campaign question, not a default.
