...
A deleted guarantee is **tombstoned in place, never erased**: `Requirement::m_removed` on the FRETISH path, `tlsf::SectionEntry::m_removed` on the TLSF one. Erasing it would shift every later requirement, and the whole codebase pairs specifications by position — `average_timing_similarity` and both semantic-similarity folds zip by index, and `tlsf_crossover` refuses two parents whose section sizes differ. A candidate that erased guarantee 0 of 3 would have its remaining two scored against the original's guarantees 1 and 2, and would lose the ability to breed. Keeping the slot makes deletion a mutation of an existing position, which is the shape every comparison already assumes.

The flag is part of `operator<`, `operator==` and `hash` on both paths, because the fitness cache keys on the whole specification: a deleted guarantee must not hash equal to the live one it replaced and inherit its score. Everything that reads a requirement list skips tombstones — lowering to LTL, the status components and the MRS walk's index map, vacuity, well-separation, the bloat cap, `all_implied_by_some`, the timing pool, the mutation and simplification pools, MUC extraction and the guarantee-part split. Crossover inherits a slot whole from the first parent whenever either side has deleted it, so crossover can neither resurrect nor delete; removal is mutation's move alone. `tlsf::SectionEntry` converts implicitly *from* `Formula` and deliberately not back, so a new read site fails to compile rather than silently ignoring the flag.
