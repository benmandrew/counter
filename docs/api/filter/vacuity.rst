...
Only assumptions are checked. Unsatisfiable guarantees make the implication unsatisfiable and are therefore already reported unrealizable, which the search punishes without help. The check short-circuits for specifications with no assumptions and treats a solver timeout as satisfiable, so a slow check never silently discards a candidate.
