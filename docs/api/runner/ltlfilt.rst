...
Called at the entry point of each external-tool wrapper (``run_ltl2tgba_for_counting``, ``RealizabilityChecker::check_realizability``, ``SatisfiabilityChecker::check_satisfiability``, and ``run_ganak_on_formula``) so that semantically equivalent formulae share a single cache entry.
