...
Intended for per-generation progress output in the genetic repair loop, where columns such as ``gen``, ``%``, ``time``, and per-filter drop counts are registered at startup and updated after each generation.
