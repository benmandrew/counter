...
``p_condition_type`` flips a requirement between its *trigger* and *continual* condition type. That order is universal. Continual implies trigger for every scope and every timing, strictly so except under ``always``, where the two coincide: a trigger fires on the rising edges of its condition, and those are a subset of the timepoints at which the condition holds. Switching to continual therefore strengthens the requirement and switching to trigger weakens it, whatever else the requirement says, so the arm needs no per-timing table.
...
At 0 none of the three keys touches the ``RandomSource``. Each reads its probability before drawing, so the breeding stream is byte-identical to what it was before the arms existed and a seeded run reproduces a pre-scope run exactly. The determinism goldens are unchanged, at 212 draws.
