...
Two limits remain. ``implies_ideal`` does not vary at all on 13 of the 20 TLSF specifications, 12 of them pinned at 0, so the quality criteria are blind across most of the corpus — ``fsm-combined`` included, where apportion's largest yield gain lands. The ``elitism_rate`` interaction went untested, pinned at 0.1 throughout, and apportion deduplicates the pool that elitism refills with copies. A campaign crossing the two would say whether the cost holds at other operating points.
...
The campaign that settles it ran on 2026-08-07 at ``nsga2-truncate``, over 600 paired FRETISH runs (4 examples, 150 seeds each) and 796 paired TLSF runs (20 families, 40 seeds each), against a rule fixed before launch. Quality is non-inferior at 0: paired ``implies_ideal`` moves by −0.023 on FRETISH and −0.008 on TLSF, both intervals excluding the +0.05 margin. Yield moves the other way on TLSF, where 0 finds a repair in 0.746 of runs against 0.714 at 0.1, on 37 discordant pairs against 11 (McNemar p = 0.0002).

What keeps the default at 0.1 is wall-clock time alone. Running at 0 costs 16.2% more on TLSF and 8.2% more on FRETISH, against a bound of 10%. The shipped default therefore trades TLSF yield for speed, and a run that cares more about finding any repair than about finishing quickly should set ``elitism_rate = 0``. Only 0 and 0.1 were measured, so nothing here speaks to intermediate rates.
...
``repaired_operators`` (off) selects between two mutation-and-crossover grammars: the one counter shipped before 2026-08-19, and the repaired one. Off is the old grammar. The key exists so that one binary can run both arms of a paired campaign. What stands behind the repairs is a local smoke test alone — 20 seeds over 4 TLSF families, where ``minepump`` moves from 1/20 to 3/20, ``arbiter-aurus`` from 10/20 to 15/20, ``rg2`` from 20/20 to 20/20 and ``lily02`` from 19/20 to 20/20. That is not a decision. The arms are unpaired, the sample is four families out of 25, and no rule was pre-registered.

Seven repairs sit behind the key.

1. ``mutate_atom_formula`` gains a third move, which grafts a drawn *anchor* onto an atom under a drawn connective, with the anchor's polarity and the operand order drawn as well. Before it, the only in-place growth rule fired at a ``Not`` node alone, so guarding a positive literal took three chained mutations through intermediates that had to survive selection.
2. ``mutate_atom_name`` prefers a distinct atom. Without that preference a rename was a no-op one draw in the pool size, which on a two-signal specification is every other draw.
3. The FRETISH graft site (``replace_subformula`` and ``combine_subformula``) is drawn uniformly over the field's nodes rather than by a fair coin at each node of a post-order walk. The old walk reached the k-th node with probability ``2^-k`` and grafted nowhere at all with probability ``2^-n`` — half the time on a single-atom field, returning the first parent's field verbatim from an operator documented as always recombining.
4. ``mutate_temporal``'s binary case gains a fourth arm that keeps the node's own connective, so an implication reached by mutation can be re-emitted rather than only destroyed.
5. ``tlsf_add_assumption`` draws its obliged literal from the inputs whatever ``allow_output_assumptions`` says, that flag now governing the guard alone. It also draws the consequent's modality from ``F``, ``X`` or none rather than hard-wiring ``F``, and flips a guard that coincides with its consequent, so the template cannot emit a tautology.
6. TLSF mutation and crossover read a slot's *section* rather than only its side. An initial condition (``INITIALLY`` or ``PRESET``) draws from its own side's signals, takes the propositional rewrite alone, and accepts a crossover donor only from its counterpart section, because basic TLSF requires both to be propositional over one side's signals.
7. TLSF crossover's graft replaces the first occurrence of the chosen subformula rather than every one, which is what its callers already documented.

It is off by default because each of the seven changes the RNG draw stream, so a seed does not reproduce across the key. An archived campaign config that omits the key would otherwise silently mean something it never ran with, the same *config vintage* reasoning that keeps ``accumulate_repairs`` off and the note in ``experiments/README.md``.

The semantics-preserving simplification folds that landed alongside the repairs are not gated and ship in both arms: ``G G φ`` to ``G φ``, ``F F φ`` to ``F φ``, and the ``U``, ``W`` and ``R`` self-joins. ``Formula::simplify()`` is called with no configuration in reach, so there is nowhere for the key to be read. Those folds are equalities, changing what a formula looks like and not what it means, which is why both arms can carry them without the comparison losing its meaning.
