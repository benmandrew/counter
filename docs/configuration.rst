...
Two limits remain. ``implies_ideal`` does not vary at all on 13 of the 20 TLSF specifications, 12 of them pinned at 0, so the quality criteria are blind across most of the corpus — ``fsm-combined`` included, where apportion's largest yield gain lands. The ``elitism_rate`` interaction went untested, pinned at 0.1 throughout — the default at the time, and apportion deduplicates the pool that elitism refills with copies. A campaign crossing the two would say whether the cost holds at other operating points.
...
What kept the default at 0.1 until 2026-08-21 was wall-clock time alone. Running at 0 costs 16.2% more on TLSF and 8.2% more on FRETISH, against a bound of 10%. That cost does not bind on the corpus counter is now measured against: runs over the AuRUS corpus use 0.3% to 2% of the 7200 s cap, so the shipped default no longer trades TLSF yield for a saving that matters. Set ``elitism_rate = 0.1`` on a run that is up against its wall-clock cap. Only 0 and 0.1 were measured, so nothing here speaks to intermediate rates.
