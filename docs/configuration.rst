...
``[filters] stream_implication`` (default ``false``) moves the final implication filter from after the search to during it. With it on, each repair ``accumulate_repairs`` collects is pushed onto a queue as it is accumulated, and a coordinator thread of its own takes whatever has queued at each wake-up and merges it into the maximal set found so far, running the weakening screen over each queued specification first where ``run_weakening`` is on. The coordinator's solver checks run on the thread pool's background queue, where the search's own scoring and filters run on the foreground one; a worker takes a background task only when the foreground queue is empty, so the search waits on at most the background checks a worker already holds and never on one still queued. What remains to merge once the search ends is the last batch plus any realisable specification the final population's own collection found that no generation had already accumulated, and ``repair_N.json`` / ``repair_N.tlsf`` are unchanged — the same maximal set, numbered in the same order a batch filter would give. The key is inert unless ``accumulate_repairs`` and ``run_implication`` are both on, and on the TLSF path it also needs ``repair_mode = "monolithic"``, MUC mode accumulating nothing.

Nothing accumulated is deleted; every file still lands in ``<output-dir>/accumulated/``, and beside them ``accumulated/maximal.tsv`` lists the files currently maximal — a ``file`` header line, then one name per line in accumulation order — rewritten after every batch through a temporary file and rename, so a run killed mid-write keeps the previous batch's listing, and a write failure warns once and the run continues without it. Streaming reaches the same antichain as one sweep over the union wherever no implication check times out, the settled members already forming an antichain so only pairs touching a new arrival are asked; a timed-out check reads as non-implication on both paths, but the two ask different pairs, so a run with timeouts can differ. ``run.json``'s ``implication.comparisons`` and ``implication.skipped`` then count the streaming sweep and do not compare against a batch run's figures, and ``implication.duplicates`` reads 0, the stream deduplicating before a specification queues. Manifest ``k_schema_version`` went 27 to 28 for the key, and no campaign has measured it; it draws nothing from the ``RandomSource``, so the search's breeding stream is unchanged.

``[mutation]`` weights what a mutation does. ``p_trigger`` (0.5), ``p_response`` (0.5) and ``p_timing`` (0.15) select which part of a FRETISH requirement is rewritten, and ``p_condition_type`` (0) and ``p_scope`` (0) reach the two fields those three leave alone. ``p_monotone`` (0) sits inside the first two rather than beside them, deciding how the field one of those two lands on is rewritten.
...
``p_condition_type`` (0), ``p_scope`` (0) and ``p_monotone`` (0) are the three *directional* arms, and they apply on the FRETISH path alone. Each moves the requirement in a direction fixed before the draw, so a move on any of them strengthens or weakens what it rewrites. The first two are alike: each rewrites a field whose handful of values are ordered by implication, and each is offered beside the other mutation arms. ``p_monotone`` differs on both counts, rewriting a formula field and being offered inside two arms rather than beside them. All three default to 0, which means none fires unless a configuration turns it on, and no campaign has yet measured any of them.
...
``p_monotone`` is the probability that a field already selected for rewriting takes a *monotone* rewrite in place of the general one. A monotone rewrite returns a formula comparable to what it replaced under implication, where the general rewriter moves the propositional structure freely and leaves the population filters to discard whatever went the wrong way. Because the arm fires inside ``p_response`` and ``p_trigger``, it is unobservable while both of those sit at 0. It is the rewrite the TLSF path already offers under ``[tlsf.mutation] p_monotone`` (0.25), shared as ``monotone_rewrite`` and called by both front ends, so the bare name now names two keys with different defaults.

The FRETISH call site turns both of the rewrite's optional menu widenings on, where the TLSF one leaves ``monotone_atom_rules`` and ``monotone_extra_rules`` off. Those two gates exist to hold an archived draw stream byte-identical, which a new arm has nothing to preserve, and with the atom rules off the menu at a literal is the rewrite to a constant alone, which guts a propositional field rather than moving it along the order. A FRETISH condition and response are propositional by construction, so the extra rules, which fire at ``Release``, ``Next`` and a biconditional, never come up there and the propositional rules carry the path.
...
At 0 none of the three keys touches the ``RandomSource``. Each reads its probability before drawing, so the breeding stream is byte-identical to what it was before the arms existed and a seeded run reproduces a pre-scope run exactly.
