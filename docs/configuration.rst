...
``genetic.constrained_domination`` changes the ranking rule both NSGA-II schemes share, and defaults **false**. On, a candidate at the top status tier precedes one below it whatever the two similarity objectives say, two below it are ordered by how far below they sit, and two at the same distance fall through to plain dominance. That last part diverges from Deb's rule, which leaves two equally infeasible candidates incomparable: the violation here is the status objective, which takes 3 to 7 values over ``examples/``, so an incomparable tier would put every candidate sharing a status score on one front — the degenerate front the constraint exists to break up.

It exists because the original specification is permanently non-dominated. Both similarity objectives are measured against the original, so it scores 1.0 on each by construction, and nothing dominates a point maximal in two of three objectives. Instrumenting the rank-0 front at generation 10 over 21 runs — ``arbiter-aurus``, ``fsm``, ``fsm-combined``, ``fsm-timing``, ``lily11``, ``minepump`` and ``rg2`` at three seeds each, under ``nsga2-apportion`` — found the original on the front in 21 of 21, at exactly (1.0000, 1.0000) on the two similarity axes, holding a front slot and breeding every generation. The smallest ε-dominance margin that would displace it runs from 0.0010 on ``fsm-timing`` to 0.3636 on ``lily11``, a factor of 364, so no single margin covers the corpus; a constraint needs no margin.

It is off by default because it also removes the similarity-against-status trade from the front, which is what ``2026-07-24-ablation`` and ``2026-08-29-aurus-matched`` credit NSGA-II's quality edge over ``weighted`` to. ``experiments/constrained-domination`` (TLSF sweep K) is the campaign that measures the exchange; until it closes this is an arm rather than a recommendation. The key reads nothing from the ``RandomSource`` and asks no solver query, so at ``false`` the ranking and the breeding stream are byte-identical to what they were before it existed.
...
``[mutation]`` weights what a mutation does. ``p_trigger`` (0.5), ``p_response`` (0.5) and ``p_timing`` (0.15) select which part of a FRETISH requirement is rewritten, and ``p_condition_type`` (0) and ``p_scope`` (0) reach the two fields those three leave alone.
...
``p_condition_type`` (0) and ``p_scope`` (0) are the two *directional* arms, and they apply on the FRETISH path alone. Each rewrites a field whose values are ordered by implication, so a move on either strengthens or weakens the requirement in a direction known before the draw. Both default to 0, which means neither fires unless a configuration turns it on, and no campaign has yet measured either.
...
At 0 neither key touches the ``RandomSource``. Each reads its probability before drawing, so the breeding stream is byte-identical to what it was before the arms existed and a seeded run reproduces a pre-scope run exactly. The determinism goldens are unchanged.
