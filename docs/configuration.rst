...
It is on by default. Turning the flag off relaxes search pressure rather than admitting vacuous repairs: collecting the realizable survivors applies the same predicate regardless of this flag, on both paths, and asks it before the synthesis query, so a vacuous candidate never costs an ``ltlsynt`` call.
