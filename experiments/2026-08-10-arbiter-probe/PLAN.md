...
**Second superseded execution.** The re-run at `df66e44` also completed, and was also discarded. `fb4c3ed` carried a defect that this campaign was the first to exercise: a formula folded to SPOT's constant spelling by the new weak-until rewrite reached `black`, which rejects `0` as a syntax error, and the run aborted. It cost 66 of 800 runs. The losses are not missing at random — a run dies only where the search reaches such a formula, so the rate is a function of what each scheme explores, and it fell unevenly (38 `nsga2-apportion` against 28 `nsga2-truncate`, with `round-robin-arbiter` down from 40 paired seeds to 23). Dropping unpaired rows would have biased the paired comparison this campaign is built on. `b093374` fixes it. As above, none of that execution's numbers appear in this file, and the decision rule in §7 is unchanged from its pre-registration.
