...
**Correction, 2026-08-27.** Those two figures were not measured over the same thing. counter's `best_relation` is a maximum over its maximality-filtered `repair_N` output, `run_implication_filter` defaulting true. AuRUS's was a maximum over its unfiltered realizable and well-separated solution set, a mean of 317.7 solutions per run against counter's 37.9. The *maximality filter* migrates a run from strictly weaker to incomparable whenever a weaker solution is dominated by an off-axis one, so the unfiltered AuRUS side could not read incomparable at counter's rate by construction.

The AuRUS side was rescored on 2026-08-27 through counter's own maximality filter (`scripts/aurus_maxscore.py` in this archive, writing `maxscore-av2.csv` and `maxscore-av3.csv`). Over 203 scored cells spanning the 20 families both tools cover, 212 attempted of a possible 600 with 8 lost to a maximality timeout and 1 skipped, the relation mix moves from 114 equivalent / 55 strictly weaker / 31 strictly stronger / 3 incomparable to 125 strictly stronger / 46 incomparable / 19 equivalent / 13 strictly weaker. AuRUS reads incomparable on 46 of 203 (22.7%), against counter's 150 of 400 (37.5%) on the same 20 families in the 2026-08-21 ship campaign. The gap is about 15 points rather than the ~45 the original pair of figures implied.

counter remains the more incomparable of the two, so section 5's argument and the monotone-rewrite operator's premise both hold. `implies_ideal` is invariant under the filter on 202 of the 203 cells, 0.714 unfiltered against 0.709 filtered. Only the relation mix was misread, and this report's endpoint conclusions stand.

Two limits sit on the rescore. The AuRUS side is a partial sample, 203 of roughly 600 cells for those families, and the counter side is drawn from the 2026-08-21 ship campaign rather than from this campaign's own 499 runs. A third scoring in the same CSVs, one representative per implication-equivalence class, gives a relation mix identical to the maximal one.
