...
Why 9x the search budget appears to buy nothing has since been answered, post hoc and on this campaign's own rows, so it is recorded here rather than left owed. The pooled 150 to 154 at p = 0.6358 is two large opposite effects cancelling. Split the 23 families by whether the `-l` arm finishes inside the 7200 s cap and the 19 that do read 131 to 153, 22 gained and none lost, exact McNemar p = 4.8e-7; the four that do not — `full-arbiter-aurus`, `humanoid-503`, `pcar-v2-888` and `prioritized-arbiter-aurus` — read 19 to 1, and 43 of their 44 large runs emitted no repair at all. The extra budget converts wherever it is allowed to finish, and the losses are censored runs rather than worse searches, the `-s` arm timing out on 0 of the same 44.
