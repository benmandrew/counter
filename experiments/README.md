...
`genetic.elitism_rate` crossed the line on 2026-08-21, moving from `0.1` to `0.0`. Unlike the keys above it existed throughout, so the exposure is exactly the archived configs that omit it — which is nearly all of them, `2026-08-07-elitism` excepted, whose two arms write the key into both. Write `elitism_rate = 0.1` in by hand to reproduce anything else. Omitting it does not shift a threshold: the top tenth of every generation no longer carries over verbatim, so a tenth of each generation that used to bypass crossover, mutation and the offspring filter chain is now bred and screened like the rest. That changes the population the campaign measured from generation 1 onward, and it changes wall time — the A/B that closed here put 0 at 16.2% more on TLSF and 8.2% more on FRETISH. What moved the default was the other half of that same campaign: quality non-inferior at 0 and TLSF yield better, 0.746 against 0.714 at McNemar p = 0.0002. The wall-clock cost was the whole reason to keep 0.1, and it stopped binding once counter was measured against the AuRUS corpus, where a run uses 0.3% to 2% of its 7200s cap.
