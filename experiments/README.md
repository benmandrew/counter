...
| `2026-08-07-elitism` | **Running.** Whether the shipped `genetic.elitism_rate` default should be 0.0 or 0.1 — sweep R on its own, `nsga2-truncate` only, 1,200 FRETISH runs (4 specs, 150 seeds, gen40/pop1000) and 1,600 TLSF runs (20 fixes-backed families, 40 seeds, gen10/pop200) across av2 and av3. It re-tests a factor `2026-07-31-replicate` already carried as a nuisance variable, whose 6,000 recovered rows show quality and yield flat and cost favouring 0.1 by 12–31%; those rows predate the tautology screen (`3869c53`) and the flip of `run_well_separation` to true (`86e463c`), and elites bypass the filter chain that applies it. The decision rule is pre-registered in its `PLAN.md`, and is the first in this directory written to leave a deliverable either way: if 0.1 is kept, the `n=1` lily02 anecdote currently cited in `include/config.hpp` has to be replaced by whichever criterion actually failed. |
