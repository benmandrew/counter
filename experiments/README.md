...
| `2026-08-19-accumulator` | **Decision: the pre-registered rule fired in the accumulator's favour, so `[genetic] accumulate_repairs` becomes a candidate default**, subject to the FRETISH replication its `PLAN.md` names. That replication has not been run: the default was flipped to `true` on 2026-08-25 on this campaign's TLSF evidence alone, a departure from its own condition that `default_flip` in `PROVENANCE.json` records and that leaves the replication owed. counter otherwise emits the maximal antichain of its final population alone, so a gate-passing candidate from an earlier generation that was not selected forward is a repair the search found and discarded; the key keeps every one of them, deduplicated and uncapped, unioned with the final population's collection before the final filters run. Sweep N crosses `accoff` against `accon` over 200 runs on the 25-family `H2H_TLSF_READY` corpus at 4 seeds split across av2 and av3, gen10/pop200 `nsga2-apportion` log metric, 240 s per-spec cap, `jobs = 8`, aurus-h2h's operating point, paired by `(spec, seed)`. Exact McNemar on per-run `implies_ideal` finds 6 discordant pairs, all to `accon`, p = 0.0312, with yield 79 against 81 of 100. Cost clears the pre-registered bound of 1.25 at a median paired wall ratio of 1.034 (mean 1.055, max 1.51), and timeouts are identical at 16 per arm, so the cap censored both alike. Over 84 `accon` runs the accumulator contributed a median of 40 repairs the final population did not hold, and contributed nothing on 2. `n_repairs` is the wrong headline for the change: it rose on 46 pairs, held on 51 and fell on 3, and the falls are not regressions, the count being the maximal antichain after the implication filter, so an accumulated repair that dominates several final-population members collapses them into itself — `simple-arbiter-aurus` seed 1 shrank 7 to 4 and gained `implies_ideal`, making it simultaneously one of the six wins. The 240 s cap sits between aurus-h2h's p75 (161 s) and p90 (684 s) on the same corpus, so this cannot say whether the advantage grows with the budget; TLSF only, `repair_mode = "muc"` inert by design, and 4 seeds is thin per family, so no per-family claim should be read off it. `campaign/accumulator` merges the unmerged `feat/aurus-h2h`, so its commits are not ancestors of `main` and `provenance/accumulator-campaign` is what holds them. |
| `2026-08-21-aurus-h2h-ship` | **Decision: no separation (`PLAN.md` §5 outcome 3), reported as measured with no re-run and no post-hoc arm.** The re-match of `2026-08-14-aurus-h2h` at counter's 2026-08-21 shipping configuration plus `accumulate_repairs = true`, one arm over the same 25 TLSF families, 20 seeds, 7200 s cap and host split, against that campaign's 780 archived AuRUS rows unchanged and through the same `analyse_aurus_h2h.py`. Per-family Wilcoxon signed-rank puts counter higher on 10 and AuRUS on 7 with 8 tied, W+ = 83.5, p = 0.7549, mean rate 0.502 against 0.504; clustered to the 10 problem types it is 5, 4 and 1 tied, W+ = 17.5, p = 0.5898. The 0.222 mean gap that decided 2026-08-14 is 0.002 and its p = 0.0127 does not reproduce, so the four changes made permanent since — the accumulator, the repaired mutation and crossover grammar, and the weakening screen defaulting off — close the gap without opening one the other way. The registered hazard of §6 fired: ties rose from 6 families to 8 and n fell from 19 to 17 because counter improved onto families where AuRUS already sat at the ceiling, so the sign and tie counts are reported beside every p. Secondary and not the endpoint, the paired counter-against-counter read over the 499 `(spec, seed)` cases both campaigns ran gains `implies_ideal` on 125 and loses it on 15, exact McNemar p = 8.9e-23 — a large within-counter move and a flat between-tool test being consistent once a family at the ceiling contributes a tie. Yield 466/500 against 447/499, timeouts 34/500 against 19/499 at the cap, median wall 64.7 s against 21.6 s for 106.0 machine-hours against 59.2, median `n_repairs` 32 against 4, and zero `compare` timeouts against 1, the budget having been raised from 600 s to 1800 s for exactly that. counter still loses `gyro-var2` (0.100 against 0.700), `lily11` (0.250 against 0.833), `lift` (0.000 against 0.300) and `minepump` (0.750 against 1.000), and leads most where AuRUS emits nothing at all, `full-arbiter-aurus` at 0.550 and `prioritized-arbiter-aurus` at 0.500. One difference between the two counter arms is not among the four changes: `run_well_separation` took the binary default of `false` here, where the 2026-08-14 arm ran the per-generation filter on because `--pin-vintage` writes that key from a `gen_configs.DEFAULTS` entry stale since `b101ada`. `experiments/monotone` is the follow-up, three arms over this corpus with these 500 rows as its outer control. |
...
| `2026-08-26-selection-smoke` | **Decision: no change to the default; the pre-registered primary is null and `PLAN.md` §7 deliberately registered no decision rule.** `WeightedAverage` against the shipped `nsga2-apportion`, `selection_scheme` the only config key differing between the two emitted TOML files, over 21 TLSF families at 16 seeds, 672 rows, 336 paired `(spec, seed)` cases, gen10/pop200 sweep T `monoon`, log metric, weakening screen off, `jobs = 8`. The corpus is `H2H_TLSF_READY` minus four families excluded on measured cost alone — `humanoid-742` at a 7200 s mean, `humanoid-531` 6149 s, `pcar-v2-888` 1643 s, `humanoid-503` 685 s, together 84% of a seed's serial cost — a rule stated in `SELECTION_SMOKE_EXCLUDED` and never touching outcome, though three of the four sat at `implies_ideal` 0.10 under the control so the excluded set is nearly all headroom this cannot see. Primary `implies_ideal` 0.6071 against 0.5952, exact McNemar 19 against 15 discordant, p = 0.607591; `found_repair` saturated at 0.9970 in both arms with one discordant pair each way, so the 2026-07-24 ablation's yield advantage for `weighted` (87.3% against 76.4%) is untestable here and its absence is not a reversal, which `PLAN.md` §6 registered in advance. That ablation's roughly 2:1 quality deficit (14.9% against 23.7% at `nsga2-truncate`) does not reproduce, and `weighted` had never been crossed against `nsga2-apportion` at all, the truncate/apportion split postdating its retirement as the default. The one separation is cost and it was never registered: `weighted` takes 2.673x the wall time and is slower on 21 of 21 families, exact sign test p = 1e-6, per-family ratio 1.03x to 3.87x. The per-run manifests put that in `ltlfilt` rather than `ltlsynt` — on `simple-arbiter-aurus` `ltlfilt` seconds go 94.85 to 447.54 against a wall ratio of 3.87x while `ltlsynt` calls move 1.22x — with `n_accumulated_repairs` 3-5x higher, which reads as the 0.7 status weight ranking the arm for realizability so more candidates clear the gate, each costing an `ltlfilt` exec on a cache miss; the cost finding is therefore entangled with the chosen weights and need not hold at 0.2/0.5/0.5. This is the first campaign to state its fitness weights rather than inherit `gen_configs.py`'s pinned 0.33 triple, through the `--weights` flag added in `921ce1b`, and a C++ trace plus the validity check below establish that those weights reach the `weighted` arm alone: the fresh control arm agrees with the `monoon` rows of `2026-08-23-monotone` on 335 of 336 runs (99.7%), wall within 3.6%, `n_repairs` within 0.1%, across a 0.33-against-0.1/0.2/0.7 difference — which also shows the engine did not move between `6b78709` and `921ce1b`. One censored run, one-sided, `prioritized-arbiter-aurus` seed 15 in the `weighted` arm, accounting for 1 of the control's 19 wins. `lily11` is the one family going the other way (0.31 to 0.75, 8 against 1 discordant, p = 0.0391 uncorrected and 0.820 Bonferroni over the 21 families the design licensed) and is a lead, not a finding. A fitness-weight sweep and `lily11` at more seeds are owed; TLSF only. |
| `2026-08-28-mrs-ordering` | **Decision: `mrs_admission_order` stays at `degree`, unchanged; both measured alternatives are refused.** Not a campaign and carries none of the apparatus of one — no arms, no seeds, no hosts, no results CSV, and nothing pre-registered, so there is no `PLAN.md`, no `campaign.toml` and no `PROVENANCE.json`, and `REPORT.md` names its commits inline (`30e8dd2` for the degree and optimum measurements, `9258ef4` on av2 for the mutant and cost ones). Asks what the pairwise conflict degree behind `conflict_degree_order` separates across the 38 TLSF examples and 383 guarantee parts, and tabulates every example's parts grouped by degree. It separates nothing on 15 of 38 families, where every part shares one degree and the sort returns index order; 8 of those hold three or more parts at degree 0 throughout (`amba` at 52 parts, `pcar-v2-888` at 24, `humanoid-531` and `lift` at 16 each, `humanoid-458`, `lily16`, `arbiter-handshake`, `lily15`), 138 parts in total, where no pair conflicts and the whole specification is still unrealizable. A triple probe puts those conflicts at arity four or more: `lift` and `humanoid-531` have 0 conflicting triples of 560 each and `arbiter-handshake` 0 of 10, while their walks still reject 3, 1 and 1 parts. `humanoid-741` is the mirror, all 30 parts unrealizable alone, every degree 29 and the walk keeping nothing. Where it does fire, 11 families carry a single part of degree >= 2, `detector` at the extreme with one part at degree 6 against six at degree 1, taking the walk from 1 part of 7 to 6. Exact maxima were computed for all 38 by descending-cardinality search with the solo and pairwise verdicts pruning by antitonicity, 5,381 queries, `amba` the worst at 24.3 s: degree order attains the exact maximum on 34 of 38 inputs against index order's 29, the whole remaining gap sitting in `lift` (13 against 15), `prioritized-arbiter` (8 against 9), `prioritized-arbiter-aurus` (9 against 10) and `lily15` (3 against 4). That gap does not survive replay onto 300 mutants over 10 families: index to degree is 67 better and 0 worse, replicating the header claim in `include/fitness/status.hpp`, while degree to an input-optimal order is 13 better and 3 worse for a pooled mean of 0.603 -> 0.606. Scoring each candidate under several fixed orders and keeping the best moves the mean 0.0149 over 270 mutants with 31 gained and none lost, at 1.83x the realizability tool time under the best gate (extra orders only where the first walk keeps at most n-2 parts) and 7.09x ungated; redrawing the orders per candidate scores 0.6296 against the fixed portfolio's 0.6297 for 22% more cache misses, so a portfolio belongs drawn once and replayed. Cost of the shipping order is 0.050% of a campaign's median run wall time (median 0.080 s, max 1.587 s on `amba`), which is why the refusals rest on the gain rather than on the price. Every endpoint here is the MRS status score, a lower bound on the true maximum realizable subset rather than a repair outcome, so the campaign that would convert any of this into `implies_ideal` or yield is owed and unrun. Its three tools are vendored as `scripts/degrees.cpp`, `scripts/orders.cpp` and `scripts/mutants.cpp`, the second campaign archive to hold C++ sources. |
| `2026-08-29-aurus-matched` | **Decision: none registered (`PLAN.md` §2) and none taken; six anytime metrics reported as measured, every p-value post-hoc.** The 2x2 of `selection_scheme` (nsga2-apportion, weighted) against `fitness.status_grading` (mrs, aurus) at AuRUS's own budget rather than counter's: `termination = individuals`, `max_individuals = 1000`, population 100, over 25 TLSF families at 30 seeds, 3000 runs across av2 and av3 with the seed split in `campaign.toml`. The 1000-individual cap bound on every one of the 2685 manifests and the 500-generation ceiling on none. 315 runs (10.5%) were killed at the 7200 s harness cap and cost 54.7% of the 1152 core-hours, humanoid-742 on 120 of 120; a killed run keeps its accumulated candidates and nothing else, so its curves exist and its verdict does not. Post-hoc, nsga2-apportion finds a repair on 122 pairs where weighted does not against 34 the other way (exact McNemar p < 0.0001), implies the ideal on 119 against 70 (p = 0.0004), and is faster on 1333 of 1500 pairs; the grading contrast is null on both (p = 0.0798, p = 0.3560) and aurus grading is faster on 1059 of 1500. Against AuRUS per family the pooled contrast is null both ways it can be read (Wilcoxon p = 0.2896 counting a kill as a failure, p = 0.5392 over scorable runs), the 2026-08-21 ship campaign having read it null at counter's budget. The two maximal metrics were scored offline on the hosts over 716 worker-hours after a 2026-09-03 rebuild of `maximal` without the `ltlfilt --simplify` pass (d4f8be4, 10x on those queries, same survivors); 280 of 3000 maximal curves are partial past a 900 s per-cut budget and are read as undecided beyond their last cut, and 205 runs have no ideal metric at all because `compare` exceeded its 600 s cap, 118 of humanoid-742's 120 among them. The `compare` fix (2defe77) exists on the branch and the archive was deliberately not re-scored with it. Its source commits are not ancestors of `main`; `provenance/aurus-matched` holds the profile commit and `provenance/aurus-matched-close` the close. |
...
`genetic.accumulate_repairs` crossed the line on 2026-08-25, moving from `false` to `true`. The exposure is every archived config that omits it, which is all of them bar the `2026-08-19-accumulator` campaign's own two arms and the sweeps that write the key explicitly, so reproducing any other campaign here means writing `accumulate_repairs = false` into the `[genetic]` table by hand. Unlike every other key in this note, the search is untouched by it. `RepairAccumulator` never draws from the `RandomSource`, so the seed stream is byte-identical at either value and only the set a run emits differs. That set moves enough to matter. It changes `n_repairs`, and it flips `implies_ideal` on a run whose good repair was lost from the final population, which are the primary endpoints of most archives here. The campaign that measured the key made it a *candidate* default, subject to a FRETISH replication that has not been run, so the flip departs from that campaign's own condition and the replication is still owed.
...
An eighth change removes a key outright and exposes nothing at all. `mutation.strengthen_assumptions` went on 2026-08-26, and unlike the sixth and seventh removals it rejects no archived config and reinterprets none: zero of the 75,646 archived `.toml` files under `experiments/` state the key. None of them could. `gen_configs.py` never emitted it and no sweep table ever crossed it, and its own doc comment said it was "retained as a flag only so the two directions can be crossed as an experiment factor", which none of the 19 archived campaigns did. The flag governed the direction in which the FRETISH assumption-side timing rewrite mutates, and at its default of `true` that direction follows the side, weakening a guarantee and strengthening an assumption. That default is now hard-wired in `src/genetic/mutation.cpp`. Every campaign archived here ran at it, so every one of them reproduces bit-for-bit, RNG stream included; the flag was read outside the draw sequence and never cost or consumed a draw. The TLSF path already ignored it entirely, `tlsf_monotone_rewrite` drawing its direction as a fair coin and deliberately not reading the key. `filters.run_weakening` above exposes nothing because every archived config pins it explicitly, and this key exposes nothing for the opposite reason, no archived config being able to name it at all.
