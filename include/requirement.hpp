...
enum class Timing {
    Immediately,    ///< Response must hold immediately when trigger is true
    NextTimepoint,  ///< Response must hold at the next timepoint after trigger
    WithinTicks,    ///< Response must hold within N ticks of trigger
    ForTicks,  ///< Response must hold for N consecutive ticks after trigger
};
...
struct Requirement {
    std::string
        m_trigger_name;  ///< The trigger condition (propositional formula)
    std::string
        m_response_name;  ///< The response obligation (propositional formula)
    Timing m_timing;      ///< The timing constraint for satisfaction
    std::size_t m_tick_count =
        0;  ///< Tick count parameter for WithinTicks/ForTicks timing
};
...
struct State {
    bool m_trigger_holds = false;   ///< Whether the trigger condition holds
    bool m_response_holds = false;  ///< Whether the response condition holds
    bool m_countdown_state =
        false;  ///< Countdown mechanism state (for timed constraints)
    std::size_t m_countdown_ticks = 0;  ///< Remaining ticks in countdown
