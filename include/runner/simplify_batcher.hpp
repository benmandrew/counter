...
///        per formula.
///
/// Reachable only under `simplify_engine = "ltlfilt"`. The default in-process
/// engine never spawns, so it never batches, and `ltlfilt_batchers` has no
/// effect there.
///
/// Separate from ltlfilt.cpp because the two are separable concerns: that file
/// selects an engine and handles a lock that may be busy, this one runs a
/// leader election over a pipe pair. Keeping both in one translation unit had
/// taken it to the cognitive-complexity limit the project sets for itself.
...
/// formula cannot be batched, `ltlfilt` could not be spawned, or the batch
/// failed its line-count check. A caller that gets nothing back is expected to
/// fall back to its own exec rather than treat it as a failed simplification.
///
