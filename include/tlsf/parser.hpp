...
///
/// Accepts the `INFO { ... } MAIN { ... }` structure of basic TLSF: INFO keys
/// (TITLE, DESCRIPTION, SEMANTICS, TARGET, and any ignored extras), and MAIN's
/// INPUTS/OUTPUTS lists plus the specification subsections (INITIALLY, PRESET,
/// REQUIRE, ASSUME/ASSUMPTIONS, ASSERT/INVARIANT(S), GUARANTEE(S)). Bounded
/// operators are expanded into X-chains at parse time.
///
