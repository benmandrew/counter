...
A plain script, no pytest, matching `test_experiment_paths.py` in everything but its failure handling: this suite records every failure and reports them together at the end, exiting non-zero after the whole run rather than at the first one. Read the closing summary, not the first `FAIL` line — exiting early hid 79 of the 227 assertions behind one ambient-state failure, which is why the two suites differ here. It never touches a lab machine: the remote protocol runs against captured marker output, `collect` runs against throwaway checkouts, and the stage and queue paths run against temporary git repositories with `COUNTER_RUNNER_CMD` pointed at a stub that records its arguments. Anything added to the launch paths is testable the same way, and must be — the alternative is testing it on a machine with somebody's campaign on it.
