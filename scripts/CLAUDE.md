...
`dequeue` is how a campaign stops. It moves the entry to `cancelled`, a state no tick picks up, and where the entry is `running` it kills the tick first and the runner and `counter` processes under it second, in one script and in that order. The order is the whole of it: a tick writes the entry's state from the copy it holds in memory at the end of every phase, so an entry rewritten under a live tick reads cancelled for the length of the phase and is then overwritten by a process that never heard of the cancellation. The children are enumerated before the tick dies, since they reparent to init the moment it does and `pgrep -P` stops finding them, and killed after it, since the runner outlives the tick that started it and goes on spending the machine on rows nobody is waiting for. The entry is tombstoned rather than deleted: the log is the record of what it did, and `enqueue` numbers entries from the names already present, so a deleted 021 lets the next campaign take a number that already means something in a log somebody may read. `requeue` puts a cancelled entry back, so this is a stop and not a withdrawal, and `--dry-run` names the processes it would kill and touches nothing. Use it rather than editing queue TOML on a host by hand; everything that acts on a campaign goes through `campaign.py`, and an entry edited under a live tick is the failure the kill ordering exists to prevent.
