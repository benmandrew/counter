...
call, where the maximality sweep over the same 3000 rematch runs cost 311.7
worker-hours. `ideals = "off"` drops the `compare` call behind
`ideal_solutions`, which is 7.2 s of a 7.3 s epsilon-only run on a
421-candidate directory and re-derives what the maximality pass already
recorded. A phase with `maximality = "off"` and no `epsilon` is refused,
having nothing to score. The word count and seed must match across any two
phases whose curves are compared: a fingerprint is a bit per sampled lasso
word, and its distance to another is only a distance if both were drawn from
one sampling. A phase states every budget on the scorer's command line, so
