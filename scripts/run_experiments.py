...
# `schemes` picks which experiments/configs/<scheme>/ directories are read.
...
# level is byte-identical to the canonical A/gen10 baseline. A canonical run
# executes at most once and emits one CSV row per requested alias — the rows
# differ only in sweep/level_name/level_value.
#
...
def verify_aliases(configs_by_key: dict[tuple[str, str, str], Path],
                   scheme: str) -> dict:
    ...
    for alias, canon in BASELINE_ALIASES.items():
        a_path = configs_by_key.get((scheme, *alias))
        c_path = configs_by_key.get((scheme, *canon))
        if a_path is None:
            continue  # alias config not generated; nothing to alias
        if c_path is None or a_path.read_bytes() != c_path.read_bytes():
            print(f"WARN: {scheme}/{a_path.name} is not byte-identical to "
                  f"canonical sweep_{canon[0]}_{canon[1]}.toml — treating as "
                  f"distinct")
            continue
        active[alias] = canon
    return active
...
def scheme_of(config_path: Path) -> str:
    """Selection scheme from the config's parent directory."""
...
def run_one(config_path: Path, sweep: str, level_name: str, spec_name: str,
            seed: int, timeout: int, parallel_k=None,
    ...
    the caller passes one that includes the selection scheme, since the same
    (sweep, level, spec, seed) is run once per scheme and the two would
    otherwise share a directory and read each other's repair_*.json.
    """
...
def main() -> None:
    ...
    def _config_sort_key(p: Path):
        ...
        return (scheme_of(p), sweep, 0 if numeric else 1,
                level_value if numeric else 0, str(p))

    wanted_schemes = profile["schemes"]
    all_configs = sorted(
        (c for s in wanted_schemes for c in (CONFIGS_DIR / s).glob("sweep_*.toml")),
    ...
    configs_by_key = {(scheme_of(c), *extract_metadata(c)[:2]): c
    ...
    active_aliases = {s: verify_aliases(configs_by_key, s) for s in wanted_schemes}
    ...
    for cfg in selected_configs:
        ...
        sweep, level_name, _ = extract_metadata(cfg)
        canon = active_aliases[scheme].get((sweep, level_name),
                                           (sweep, level_name))
        for spec_name in specs:
            for seed in seeds:
                ...
                if canon != (sweep, level_name):
                    n_aliased += 1
                key = (scheme, canon[0], canon[1], spec_name, seed)
    ...
    n_done = sum(
        ...
        if (sweep, level_name, key[0], key[3], key[4]) in done
    )
    to_execute = [
        key for key, row_list in runs.items()
        if any((s, l, key[0], key[3], key[4]) not in done for s, l in row_list)
    ]
    ...
    if args.dry_run:
        for key, row_list in runs.items():
            scheme, c_sweep, c_level, spec_name, seed = key
            for sweep, level_name in row_list:
                tags = []
                if (sweep, level_name) != (c_sweep, c_level):
                    tags.append(f"(alias of {c_sweep}/{c_level})")
                if (sweep, level_name, scheme, spec_name, seed) in done:
                    tags.append("(skip)")
                print(f"  {scheme}/sweep_{sweep}_{level_name}  spec={spec_name}"
                      f"  seed={seed:02d}"
    ...
    def execute(key: tuple) -> None:
        ...
        with lock:
            ...
            for sweep, level_name in runs[key]:
                ...
                done.add((sweep, level_name, scheme, spec_name, seed))
