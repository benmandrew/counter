...
// How long the child gets to exit on its own once it has closed its stdout,
// before its process group is killed. It has already written every answer by
// then, so this only bounds a child that lingers after its last write.
constexpr std::chrono::milliseconds k_reap_grace{2000};
...
// and stdbuf cannot reach it, so a request/response protocol over a long-lived
// child deadlocks. Closing stdin after the whole batch is what makes it emit
// everything.
bool run_ltlfilt_batch(const std::string& binary,
    ...
    // KillWithParentThread, the same policy as any one-shot tool: this child is
    // spawned, fed, drained and reaped by one leader thread inside this call,
    // which is exactly the shape PDEATHSIG is tied to. A batcher child that
    // escaped it would be the worst orphan of the lot, since it holds a pipe
    // other threads are waiting on.
    const PipedChild child =
        spawn_piped_child({binary, "--simplify", "--skip-errors", "-F", "-"},
    ...
    while (written < input.size()) {
        const ssize_t bytes_written = write(
            child.m_write_fd, input.data() + written, input.size() - written);
        if (bytes_written > 0) {
    ...
    auto [output, timed_out] =
        read_until_eof(  // NOLINT(clang-analyzer-unix.BlockInCriticalSection)
            child.m_read_fd, timeout);
    if (timed_out) {
        // Killed before the reap below, which would otherwise wait on exactly
