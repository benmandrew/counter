...
/// @file fingerprint_prefilter.hpp
/// @brief The sampled-word prefilter both TLSF implication sweeps use.
///
...
/// words. The pairwise sweep in filter.cpp and the equivalence quotient in
/// maximal.cpp both prefilter, and their fingerprints have to agree.
...
/// one lowering does not render to a string this codebase's own parser reads
/// back. Losing the whole table costs speed and nothing else, where a partial
/// one would need a per-entry guard at every use.
